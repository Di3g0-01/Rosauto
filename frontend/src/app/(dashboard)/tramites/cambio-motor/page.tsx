...
export default function CambioMotorPage() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      await axios.post("http://127.0.0.1:3002/tramites/cambio-motor", {
        numero_tarjeta: selectedTarjeta,
        motor_nuevo: data.motor_nuevo,
        descripcion: data.descripcion
      });
      toast.success("Cambio de motor realizado con éxito");
      router.push("/tramites");
      router.refresh();
    } catch (error) {
      console.error("Error changing engine:", error);
      toast.error("Hubo un error al realizar el cambio de motor");
    } finally {
      setLoading(false);
    }
  };
  ...
  const tarjetasOptions = tarjetas
    ...
    .map((t: any) => ({
      ...
      label: `${t.numero_tarjeta} - ${t.vehiculo?.placa} (Motor actual: ${t.vehiculo?.num_motor})`
    }));
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Cambio de Motor</h1>
          <p className="text-zinc-500 dark:text-zinc-400">Actualización del número de motor en el sistema.</p>
        </div>
      </div>
      ...
      <div className="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-800/50 p-8 rounded-3xl shadow-xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium">Nuevo Número de Motor</label>
            <input 
              name="motor_nuevo" 
              required
          ...
          >
            {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><CarFront className="mr-2 h-4 w-4" /> Registrar Cambio de Motor</>}
          </Button>
        </form>
      </div>
    </div>
