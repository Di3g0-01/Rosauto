...
export default function TraspasoPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([resTarjetas, resPropietarios]) => {
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    if (!selectedTarjeta || !selectedPropietario) {
    ...
    try {
      await axios.post("http://127.0.0.1:3002/tramites/traspaso", {
        numero_tarjeta: selectedTarjeta,
        nuevo_nit_cui: selectedPropietario,
      ...
      console.error("Error performing traspaso:", error);
      toast.error("Hubo un error al realizar el traspaso");
    } finally {
      setLoading(false);
    }
  };
  ...
  const tarjetasOptions = tarjetas
    ...
    .map((t: any) => ({
      ...
      label: `${t.numero_tarjeta} - ${t.vehiculo?.placa} (${t.propietario?.nombres} ${t.propietario?.apellidos})`
    }));
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Nuevo Traspaso</h1>
          <p className="text-zinc-500 dark:text-zinc-400">Cambio de propietario de un vehículo.</p>
        </div>
      </div>
      ...
      <div className="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-800/50 p-8 rounded-3xl shadow-xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium">Nuevo Propietario</label>
            <CustomCombobox
          ...
          >
            {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><ArrowRightLeft className="mr-2 h-4 w-4" /> Registrar Traspaso</>}
          </Button>
        </form>
      </div>
    </div>
